...
export const GlutenFree = () => {  
  ...
  let intolerance = types.uiGlutenFree; 

  useEffect(() => {
      setTimeout(() => {
          setOffset(24);
          sethasMore(true);
          getRecipesBySearch(search, intolerance, '')
              .then( res => {
  ...
  const getData = () => {    
      setTimeout(() => {
        getRecipesBySearch(search, intolerance, '', offset)
            .then( res => {
  ...
  return (
    ...
    <div className='container-general'  id="scrollableDiv" style={{ height: '90%' }}>
      ...
      <Divider />   
      <Typography variant='h4' className='text-title-filter'>Gluten Free</Typography>
